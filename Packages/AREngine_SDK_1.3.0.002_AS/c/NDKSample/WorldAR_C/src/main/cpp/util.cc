...
namespace hello_ar {
    namespace util {
        ...
        bool LoadPngFromAssetManager(int target, const std::string &path) {
            ...
            static struct JNIData {
                ...
                constexpr char kHelperClassName[] =
                        "com.huawei.arengine.examples.c.helloar/JniInterface";
        ...
        bool LoadObjFile(AAssetManager *mgr, const std::string &file_name,
            ...
            // files (.obj).
            //LOGE("LoadObjFile AAssetManager_open start!");
            AAsset *asset =
            ...
            while (!file_string_stream.eof()) {
                ...
                if (line_header[0] == 'v' && line_header[1] == 'n') {
                ...
                } else if (line_header[0] == 'v' && line_header[1] == 't') {//vt read
                    // Parse texture uv.
                    GLfloat uv[2];
                ...
                } else if (line_header[0] == 'v') { //v read
                    // Parse vertex.
                    GLfloat vertex[3];
                ...
                } else if (line_header[0] == 'f') { //f read
                    ...
                    //LOGE("LoadObjFile vertices_count=%d",vertices_count);
            ...
            //LOGE("LoadObjFile normal_indices.size()=%d,uv_indices.size()=%d!",normal_indices.size(),uv_indices.size());
            //LOGE("LoadObjFile vertex_indices.size()=%d,",vertex_indices.size());
        ...
        void GetTransformMatrixFromAnchor(HwArSession *ar_session,
            ...
            HwArAnchor_getPose(ar_session, ar_anchor, pose.GetArPose());
            HwArPose_getMatrix(ar_session, pose.GetArPose(),
                               glm::value_ptr(*out_model_mat));
        }


        glm::vec3 GetPlaneNormal(const HwArSession &ar_session,
                                 const HwArPose &plane_pose) {
            float plane_pose_raw[7] = {0.f};
            HwArPose_getPoseRaw(&ar_session, &plane_pose, plane_pose_raw);
        ...
        float CalculateDistanceToPlane(const HwArSession &ar_session,
            ...
            HwArPose_getPoseRaw(&ar_session, &camera_pose, camera_pose_raw);
